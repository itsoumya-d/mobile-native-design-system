...
|---|---|
| Flutter | Project-local Flutter 3.44.0: the compact visual golden passed with the pinned engine. Global Flutter 3.41.3: `flutter analyze`, eight widget/unit/accessibility/golden tests, and `flutter build apk --debug` passed. The integration-test source is analyzed and its API 36 execution is configured in GitHub Actions. Generated component and motion Dart are analyzer inputs. |
...
|---|---|---|
| Flutter | The local iOS simulator build reached CoreSimulator but failed when `AssetCatalogSimulatorAgent` could not launch. The new integration test was not executed locally because no Android emulator is connected. Linux and macOS rasterize text differently, so the compact pixel golden is tagged and runs on macOS; Linux still runs every non-pixel widget and accessibility test. | Core CI pins Flutter 3.44 for analyze, non-pixel widget tests, and Android build. Native Android CI runs the integration test on API 36. Native Apple CI runs the golden and builds the Flutter iOS simulator target. |
